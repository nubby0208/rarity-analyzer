...
//nubby
const collectionsData = require(appRoot + '/config/' + config.collections_file_name);
//nubby
const fs = require('fs');
...
collectionsData.forEach(collection => {
    ...
    if (collection.id != undefined) {
        collection.id = collection.id.toString();
    }
    if (collection.id == undefined) {
        collection['id'] = collections_count1;
    }
    if (_.isEmpty(collection.id)) {
        collection['id'] = collections_count1;
    }
    ...
    if (_.isEmpty(collection.external_url)) {
        collection['external_url'] = '';
    }
    if (_.isEmpty(collection.animation_url)) {
    ...
    db.exec(
        "CREATE TABLE "+
        collection.name+"_trait_detail_types"+ 
            "(" +
            "id INT, " +
            "trait_type_id INT, " +
    ...
    let updateTraitTypeStmt = db.prepare("UPDATE "+collection.name+"_trait_types SET punk_count = :punk_count WHERE id = :id");
    ...
    let updateTraitDetailTypeStmt = db.prepare("UPDATE "+collection.name+"_trait_detail_types SET punk_count = :punk_count WHERE id = :id");
    ...
    let insertPunkTraitContStmt = db.prepare("INSERT INTO "+collection.name+"_punk_trait_counts VALUES (?, ?)");
    ...
    collections_count1 = collections_count1 + 1;
});
